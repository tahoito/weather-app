    ...
    useEffect(() => {
        const fetchSpots = async () => {
            ...
            try {
                ...
                const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/spots/search${params.toString() ? '?' + params.toString() : ''}`;
            ...
            <div className='fixed top-5 left-0 flex items-center gap-4 w-full h-12 px-5 z-10'>
                <div className='relative w-full'>
                    <input
                        ...
                        className={`w-full bg-white rounded-[12px] font-medium transition-all ${isInputFocused ? 'px-4 py-3' : 'py-3 pl-13 pr-3'}`}
                        placeholder='検索'
                    />
                    {!isInputFocused && (
                        <Search size={24} className='absolute top-1/2 left-4 -translate-y-1/2 text-gray-400' />
                    )}
                    {isLoading && (
                ...
                <div
                    className={`absolute top-[calc(100%+8px)] right-5 flex flex-col gap-1 bg-white p-3 border border-fg rounded-[12px] transition-all duration-200 ${isFilterOpen
